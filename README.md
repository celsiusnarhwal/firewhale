...
However, some services require access to the Docker socket for various reasons. Firewhale allows you to grant
...
Firewhale supports all versions of the Docker API that are supported by Docker's
...
If Watchtower tries to update Firewhale while using it as its Docker host, it will lose access to the Docker
